...
function openLinkEventHander( event ) {
    ...
    if ( result ) {
        if ( result.length == 1 ) {
            ...
            new Notify().Render( "Query narrowed" );
            links   = [];
            let idx = 0;
        ...
        new Notify().Render( "Currently in link mode, if you need other operations, please use ESC to exit this mode" );
    }
}
...
function keyboradmap() {
    ...
    <kbd-mapping>
        ...
        <kbd-maps>
            ...
            <kbd-maps-group>
                <kbd-maps-title>reading mode</kbd-maps-title>
